...
static init_vm(const char *myKey, int myKey_length) {
//    int jitEnabled=1, largePagesEnabled=1, hardwareAESEnabled=1;
    randomx_flags flags = randomx_get_flags();
    randomx_cache *myCache = randomx_alloc_cache(flags);
    randomx_init_cache(myCache, &myKey, myKey_length);
    return randomx_create_vm(flags, myCache, randomx_alloc_dataset(flags));

}


unsigned char prepare_input(const char *h0, const char *prevh, const char *timestampBinary, const char *chunk, const char *entropy) {
    ...
    for (int i = 0; i < len_h0 + len_prevh + len_time + len_chunk + len_entropy; i++) {
        ...
        } else if (i < len_h0 + len_prevh + len_time + len_chunk + len_entropy &&
                   i >= len_h0 + len_prevh + len_time + len_chunk) {
...
int main()
{
    ...
    unsigned char preimage = prepare_input(&h0, &prevh, &timestampBinary, &chunk, &entropy);
...
//    pthread_join(thread_id, NULL);
     ...
     hash_cal(myMachine, &preimage, sizeof preimage, hash);
